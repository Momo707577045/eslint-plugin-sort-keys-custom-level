...
module.exports = {
  ...
  meta: {
    ...
    docs: {
      ...
      recommended: false,
      // 规则配套说明书
      url: 'https://github.com/Momo707577045/eslint-plugin-sort-keys-custom',
    },
  ...
  create(context) {
    ...
    ((options && options.customTarget) || []).forEach(config => {
      config.targetKeys.forEach(key => {
        ...
        customTargetKey2Level[key].push(...(config.subLevels || []))
      })
    })
    ...
    return {
      ...
      Property(node) {
        ...
        if (stack.isCustomTarget === null) {
          ...
          let levelIndex = 1;
          while (currentStack) {
            const levels = customTargetKey2Level[currentStack.parentName];
            console.log('currentStack.parentName', thisName, currentStack.parentName, levels, levelIndex)
            if (levels && (levels.length === 0 || levels.includes(levelIndex))) {
          ...
          console.log('stack.isCustomTarget', stack.isCustomTarget);
        }
